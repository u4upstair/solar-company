  ...
  return (
    <div className="liquid-glass rounded-2xl p-3 sm:p-4 flex gap-3 sm:gap-4 items-stretch flex-1 w-full max-w-none sm:max-w-[420px] hero-anim hero-fade" style={{ animationDelay: '0.9s' }}>
      ...
      <div className="flex flex-col justify-between py-1 min-w-0">
        ...
        <div className="group flex items-center gap-0 mt-2 sm:mt-3 cursor-pointer">
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen ${isNight ? 'bg-slate-950 text-white' : 'bg-white text-gray-900'} font-sans selection:bg-neutral-800 selection:text-white p-[clamp(1.5px,0.2vw,3px)] flex flex-col box-border transition-colors duration-500`}>
      {/* Navbar - Extracted from relative z-10 context to ensure portaled menu works properly with it */}
      <header
      ...
      >
        ...
        <div className="flex items-center gap-2.5">
          <Sparkles className={`w-5 h-5 flex-shrink-0 ${isNight ? 'text-[#07CCEA]' : 'text-[#CEFE49]'}`} />
          <div className="flex flex-col">
            <span className={`text-sm sm:text-base font-bold tracking-tight transition-colors duration-300 leading-none ${
              (isScrolled && !isNight) || (menuOpen && !isNight) ? 'text-gray-900' : 'text-white nav-text-shadow'
            }`}>
              LinkFlow<sup className="text-[9px] font-medium">TM</sup>
            </span>
            <span className={`text-[10px] sm:text-[11px] transition-colors duration-300 font-medium tracking-wide leading-tight mt-0.5 ${
              (isScrolled && !isNight) || (menuOpen && !isNight) ? 'text-gray-500' : 'text-gray-400'
            }`}>
              Solar Energy
            </span>
          </div>
        ...
        <div className={`hidden lg:flex items-center gap-1 sm:gap-1.5 rounded-full px-5 py-2 transition-all duration-300 ${
          isScrolled && !isNight ? 'bg-black/[0.04] border border-black/5' : 'apple-glass'
        }`}>
          {navLinks.map((link) => {
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="group hidden sm:flex items-center gap-0 cursor-pointer">
            ...
            <button className={`w-10 h-10 flex items-center justify-center rounded-full transition-all duration-300 shadow-sm flex-shrink-0 border-0 border-transparent outline-none ring-0 focus:outline-none focus:ring-0 pointer-events-none ${
              ...
              <ArrowUpRight
                className={`w-5 h-5 transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 ${
                  isScrolled && !isNight ? 'text-white group-hover:text-gray-900' : 'text-[#1f2a1d]'
                }`}
              />
            </button>
          </div>
          ...
          <button
            ...
            className={`lg:hidden flex items-center justify-center w-10 h-10 rounded-full transition-all duration-300 ${
              (isScrolled && !isNight) || (menuOpen && !isNight)
      ...
      <div className="absolute inset-0 z-0 rounded-3xl sm:rounded-[32px] overflow-hidden">
        ...
        {typeof document !== 'undefined' && createPortal(
          <div
            ...
            className={`lg:hidden fixed inset-0 z-[9998] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
          ...
          >
            <div className="flex flex-col h-full pt-28 px-6 sm:px-10 pb-10 max-w-lg mx-auto w-full justify-between">
              <div>
                {/* Logo and Tagline block in drawer */}
                <div className="mb-10 flex items-center gap-2.5">
                  <Sparkles className={`w-5 h-5 ${isNight ? 'text-[#07CCEA]' : 'text-[#CEFE49]'}`} />
                  <div className="flex flex-col">
                    <span className={`text-base font-bold tracking-tight transition-colors duration-500 ${isNight ? 'text-white' : 'text-[#1f2a1d]'}`}>
                      LinkFlow<sup className="text-[9px] font-medium">TM</sup>
                    </span>
                    <span className={`text-[11px] font-medium tracking-wide transition-colors duration-500 ${isNight ? 'text-gray-400' : 'text-gray-500'}`}>
                      Solar Energy
                    </span>
                  </div>
                </div>
              ...
              >
                <div className="group flex items-center gap-0 w-full cursor-pointer">
        ...
        <div className="flex-1 flex flex-col justify-end gap-10 mt-auto">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-12 lg:gap-16">
            <div className="max-w-xl">
              ...
              {/* CTA row */}
              <div className="group flex items-center gap-0 mt-8 cursor-pointer hero-anim hero-fade" style={{ animationDelay: '0.6s' }}>
      ...
      <footer id="contact" className="bg-black px-6 lg:px-16 py-16 lg:py-20 overflow-x-hidden">
        <div className="max-w-7xl mx-auto flex flex-col gap-12 text-left w-full min-w-0">
          <div className="grid lg:grid-cols-2 gap-10 items-start w-full min-w-0">
            ...
            <div className="flex flex-col gap-5 w-full min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-3 mt-2">
                <div className="group flex items-center gap-1 w-fit">
                  ...
                  <div className="w-9 h-9 rounded-full bg-gray-900 flex items-center justify-center transition-transform group-hover:rotate-45">
                    <ArrowUpRight className={`w-4 h-4 ${isNight ? 'text-[#07CCEA]' : 'text-[#CEFE49]'}`} />
                  </div>
                </div>
                <div className="group flex items-center gap-1 w-fit">
                  <button className={`border border-white text-white font-bold text-sm px-6 py-3 rounded-full transition-all group-hover:scale-[1.03] cursor-pointer focus:outline-none focus-visible:ring-2 ${isNight ? 'focus-visible:ring-[#07CCEA]' : 'focus-visible:ring-[#CEFE49]'} focus-visible:ring-offset-2 focus-visible:ring-offset-black`}>
                    Talk to an Expert
                  </button>
                  <div className="w-9 h-9 rounded-full bg-white flex items-center justify-center transition-transform group-hover:rotate-45">
            ...
            {/* RIGHT */}
            <div className="flex flex-col gap-6 lg:items-end w-full min-w-0 overflow-hidden">
              <div className="flex items-center gap-2">
                <Sparkle className={`w-6 h-6 ${isNight ? 'text-[#07CCEA]' : 'text-[#CEFE49]'}`} />
                <div>
                  <p className="font-bold text-xl text-white leading-none">Creaenergy</p>
                  <p className="text-xs text-gray-400 tracking-wide mt-0.5">Solar System</p>
                </div>
              </div>

              <nav className="flex items-center gap-1 bg-white/5 border border-white/10 rounded-full p-1 w-full max-w-full sm:w-auto overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                {['Home', 'About', 'Services', 'Case Studies', 'Contact'].map((item) => (
                  <a
                    key={item}
                    href={item === 'Home' ? '#' : `#${item.toLowerCase().replace(' ', '-')}`}
                    className={`${item === 'Home'
                      ? `${isNight ? 'bg-[#07CCEA]' : 'bg-[#CEFE49]'} text-gray-900 font-bold text-sm px-4 py-2 rounded-full`
                      : `text-white text-sm px-4 py-2 hover:text-[#CEFE49] ${isNight ? 'hover:text-[#07CCEA]' : ''}`} transition-colors shrink-0 whitespace-nowrap focus:outline-none focus-visible:ring-2 ${isNight ? 'focus-visible:ring-[#07CCEA]' : 'focus-visible:ring-[#CEFE49]'} focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded-full`}
                  >
                    {item}
                  </a>
                ))}
              ...
              <div className="flex flex-wrap items-center gap-6 sm:gap-8 mt-2">
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-full ${isNight ? 'bg-[#07CCEA]' : 'bg-[#CEFE49]'} flex items-center justify-center shrink-0`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-gray-400">Mail Us</p>
                    <p className="text-sm font-bold text-white">hello@sunvia.com</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 text-sm">
            <p className="text-gray-400">
              © 2025 <span className={isNight ? 'text-[#07CCEA]' : 'text-[#CEFE49]'}>Sunvia</span>. All rights reserved.
            </p>
            <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 text-gray-400">
              <a href="#" className={`hover:text-white transition-colors focus:outline-none focus-visible:ring-2 ${isNight ? 'focus-visible:ring-[#07CCEA]' : 'focus-visible:ring-[#CEFE49]'} focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded`}>Terms & Conditions</a>
              <a href="#" className={`hover:text-white transition-colors focus:outline-none focus-visible:ring-2 ${isNight ? 'focus-visible:ring-[#07CCEA]' : 'focus-visible:ring-[#CEFE49]'} focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded`}>Privacy Policy</a>
              <a href="#" className={`hover:text-white transition-colors focus:outline-none focus-visible:ring-2 ${isNight ? 'focus-visible:ring-[#07CCEA]' : 'focus-visible:ring-[#CEFE49]'} focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded`}>Sitemap</a>
              <a href="#" className={`hover:text-white transition-colors focus:outline-none focus-visible:ring-2 ${isNight ? 'focus-visible:ring-[#07CCEA]' : 'focus-visible:ring-[#CEFE49]'} focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded`}>Cookies</a>
            </div>
            <a
              href="#"
              onClick={(e) => {
                ...
                lockScrollSpy();
                window.scrollTo({ top: 0, behavior: 'smooth' });
                window.history.pushState(null, '', '#'); // updates the URL hash without a jump
                setActiveLink('#');
              }}
              className={`group flex items-center gap-1 w-fit cursor-pointer focus:outline-none focus-visible:ring-2 ${isNight ? 'focus-visible:ring-[#07CCEA]' : 'focus-visible:ring-[#CEFE49]'} focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded-full`}
            >
              <span
                className="border border-white text-white font-semibold text-sm px-5 py-2.5 rounded-full transition-transform group-hover:scale-[1.03] block"
              >
                Back To Top
              </span>
              <div className={`w-8 h-8 rounded-full ${isNight ? 'bg-[#07CCEA]' : 'bg-[#CEFE49]'} flex items-center justify-center transition-transform group-hover:-translate-y-0.5`}>
